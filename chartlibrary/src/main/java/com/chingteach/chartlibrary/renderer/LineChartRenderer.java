 ...
 *   //2019 3.27 在魅族手机上出现getHeight=16777771的情况  73行
 */
public class LineChartRenderer extends AbstractChartRenderer {
    ...
    public void onChartSizeChanged() {
        ...
        if (computator.getChartWidth() > 0 && computator.getChartHeight() > 0) {
            //2019 3.27 在魅族手机上出现getHeight=16777771的情况
    ...
    private void drawPoints(Canvas canvas, Line line, int lineIndex, int mode) {
        ...
        for (PointValue pointValue : line.getValues()) {
            ...
            /*********************************************/
            int pointRadius;
            if (line.getLastPointRadius()!=0&&valueIndex==line.getValues().size()-1)
            {
                pointRadius=line.getLastPointRadius();
            }else {
                pointRadius = ChartUtils.dp2px(density, line.getPointRadius());
            }
            final float rawX = computator.computeRawX(pointValue.getX());
            final float rawY = computator.computeRawY(pointValue.getY());
            ...
            if (computator.isWithinContentRect(rawX, rawY, checkPrecision)) {
                ...
                if (MODE_DRAW == mode) {
                    ...
                    if(pointValue.isFinish()){
                        ...
                        }else if( y < 66.6 && (y > 33.3 || y == 33.3)){
                            color = Color.parseColor("#5bb5c4");
                        } else {
                            color = Color.parseColor("#b24337");
                        }
